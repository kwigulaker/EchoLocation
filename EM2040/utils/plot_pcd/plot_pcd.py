...
def plot_pcd_clusters(seabed,outliers,clusters,centroids):
    ...
    for id in range(len(cluster_ids)):
        ...
        f = int(cluster_ids[id])
        print(f)
        # lut = array of [r,g,b,a]
        if(counts[f] > 100):
            lut[id,:] = [255*(1-f),255*f,255*f,255]
        else:
            lut[id,:] = [0,0,255,255]
...
def plot_pcd_seabed(seabed,outliers):
    outliers = np.loadtxt(outliers)
    x = np.array(outliers)[:, 0]  # x position of point
    y = np.array(outliers)[:, 1]  # y position of point
    z = np.array(outliers)[:, 2]  # z position of point
...
seabed = "../../data/seabed/0001_20210607_130222_inliers.txt"
outliers = "../../data/seabed/0001_20210607_130222_outliers.txt"
clusters = "../../data/clusters/0001_20210607_130222_labels.txt"
#centroids = None #"../../data/clusters/0001_20210607_130222_centers.txt"
plot_pcd_clusters(seabed,outliers,clusters,None)
