...
def main(src, dst, timeout, ext, logging_level):
	survey = f"E:\sisdata\grids\{src}"
	# assert os.path.isdir(survey), "Survey not found!"
	dst += f"\{src}"
	survey += "\Terrain\zMedian"
	monitor(survey, dst, timeout, ext, logging_level, n_subfolders=2)
